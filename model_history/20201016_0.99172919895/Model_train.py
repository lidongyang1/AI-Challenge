...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if loss <= self.best: # 保存最优模型权重
            self.best = loss
            encoder.save('./Modelsave/encoder.h5')
            decoder.save('./Modelsave/decoder.h5')
            np.savetxt('./Results/nmse',np.array([self.best]))
        print('\nnmse:'+str(self.best))
        return self.best
evaluator = Evaluate()
# model training
autoencoder.fit(x=x_train, y=x_train, batch_size=128, epochs=1000, validation_split=0.1,verbose=1,callbacks=[evaluator])
