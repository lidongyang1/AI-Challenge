"""
============================================================
# File: Model_define_tf.py   V4_swish
# This file is created by pnixsoft
# Email: mqzhang@mail.sdu.edu.cn
# Date: 2020-09-10
#  1. relu--> swish
#  2. decoder 6 blocks
...
keras.utils.get_custom_objects().update({'swish': layers.Activation(swish)})

def Encoder(x_ini,feedback_bits):
    B=4
    with tf.compat.v1.variable_scope('Encoder'):
        #########
        x = layers.Conv2D(2, 3, padding = 'SAME',activation="swish")(x_ini)
        x = layers.Conv2D(2, 3, padding = 'SAME',activation="swish")(x)
        ########
        x1 = layers.Conv2D(2, 5, padding='SAME',activation="swish")(x_ini)
        x1 = layers.Conv2D(2, 5, padding='SAME', activation="swish")(x1)
        ########
        x2 = layers.Conv2D(2, 3, padding='SAME',activation="swish")(x_ini)
        x2 = layers.Conv2D(8, 3, padding='SAME', activation="swish")(x2)
        x2 = layers.Conv2D(16, 3, padding='SAME', activation="swish")(x2)
        x2 = layers.Conv2D(2, 3, padding='SAME', activation="swish")(x2)
        ########
        x = layers.Flatten()(x)
...
def Decoder(x,feedback_bits):
    ...
    for i in range(8):
        x = layers.Conv2D(2, 3, padding='SAME',activation="swish")(x_ini)
        x = layers.Conv2D(8, 3, padding='SAME',activation="swish")(x)
